...
if (isset($_GET['update'])) {
    ...
    foreach ($_SESSION['cart'] as &$item) {
        if ($item['product_id'] == $id) {
            if ($_GET['update'] === 'inc' && $item['qty'] < 100) $item['qty']++;
            if ($_GET['update'] === 'dec' && $item['qty'] > 1) $item['qty']--;
        }
    }
    unset($item);
  ...
  <?php else: ?>
    <div class="grid md:grid-cols-3 gap-6">
      <div class="md:col-span-2 space-y-4">
        <?php foreach ($cart_products as $item): ?>
          <div class="flex items-center gap-4 bg-white p-4 rounded shadow">
            <img src="../<?= htmlspecialchars($item['product_image']) ?>" class="h-20 w-20 object-contain border rounded" />
            <div class="flex-1">
            ...
            <div class="text-right">
              ...
              <a href="?remove=<?= $item['product_id'] ?>" class="text-red-500 text-sm block mt-1"><i class="fas fa-trash"></i> Remove</a>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
      ...
      <div class="bg-white p-6 rounded shadow h-fit">
        ...
        <div class="flex justify-between text-sm mb-1"><span>Subtotal</span><span>₹<?= number_format($total, 2) ?></span></div>
        <div class="flex justify-between text-sm mb-1"><span>SGST (9%)</span><span>₹<?= number_format($sgst, 2) ?></span></div>
